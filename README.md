...
Both deep-learning detectors have been validated against real-world data, not just this repo's own synthetic traffic. The DGA classifier: 84.6% precision, 77.2% recall, 13.9% false-positive rate against a real published dataset of 25 malware families' actual DGA domains (`scripts/evaluate_against_real_dga_dataset.py`), with a per-family regression gate (`benchmarks/dga_family_recall_baseline.json`) checked on every run, plus end-to-end validation against a real published malware pcap (Lumma Stealer). The flow autoencoder: 100% precision, 99.8% recall, 0.00% false-positive rate against a real, held-out botnet-traffic dataset (CTU-13) after retraining on real flow data replaced a synthetic-only model that had measured a ~98% false-positive rate against the same real data (`scripts/evaluate_flow_autoencoder_against_real_data.py`). See [SECURITY.md](SECURITY.md#model-validation-against-real-world-data) for the full numbers, dataset citations, and the real bugs this validation caught and fixed.
