...
**What the dashboard shows:** a severity-sorted Incident Queue (KPI strip + filterable table) opens into a per-incident detail view with five tabs — Summary, Evidence (full alert-by-alert drill-down), ATT&CK Mapping, Analyst Actions (Acknowledge / False Positive / Confirm & Escalate), and **Kill Chain**: a condensed, chronological attack-phase timeline (`dashboard/components/ui.py`'s `build_kill_chain`/`render_kill_chain`) that groups a possibly-thousands-long related-alert list by detector type into a handful of readable phases — e.g. `Anomalous Flow ×37 → Data Exfiltration ×63` — instead of one raw alert per timeline entry. `pages/investigate.py`, `pages/network.py`, and `pages/health.py` round out ad-hoc search, a network-graph view, and pipeline health monitoring.
...
Both deep-learning detectors have been validated against real-world data, not just this repo's own synthetic traffic. The DGA classifier: 84.6% precision, 77.2% recall, 13.9% false-positive rate against a real published dataset of 25 malware families' actual DGA domains (`scripts/evaluate_against_real_dga_dataset.py`), with a per-family regression gate (`benchmarks/dga_family_recall_baseline.json`) checked on every run, plus a second, independent validation against a real UMUDGA dataset and end-to-end validation against a real published malware pcap (Lumma Stealer). The flow autoencoder: 95.8% precision, 54.5% recall, 0.63% false-positive rate — measured against a leak-free, held-out slice of all 13 CTU-13 scenarios (`scripts/evaluate_flow_autoencoder_against_real_data.py`), after broadening its training data from 3 scenarios to 12 traded a modest, disclosed FPR increase (0.21% → 0.63%) for a real recall gain (36.9% → 54.5%) once the validation gate itself was widened to cover scenarios the model could no longer have trained on. All seven detectors combine into one calibrated per-incident score (`inference/risk.py`), itself validated against the same real data: 98.3% precision catching 53.0% of real botnet connections — a real, measured lift over any single detector alone. See [SECURITY.md](SECURITY.md#model-validation-against-real-world-data) for the full numbers, dataset citations, disclosed negative results (JA4 TLS fingerprinting was investigated twice against real malware traffic; no rule shipped), and the real bugs this validation caught and fixed.
