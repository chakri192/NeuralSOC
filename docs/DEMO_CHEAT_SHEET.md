...
> **A:** "The rule-based detection, the DGA classifier, and the flow autoencoder all run out of the box today. The DGA classifier is the one we've actually validated against real-world data, not just our own simulator — a published research dataset of 25 real malware families' actual DGA domains gives us 85.2% precision, 69.8% recall, and a 12.0% false-positive rate, with real per-family variance we can show you. That's a deliberate trade-off, not a free lunch: we rebuilt the model specifically to catch dictionary-style DGA it used to miss almost entirely, and recall nearly doubled, but false-positive rate roughly doubled too and a couple of character-random families dipped. The flow autoencoder is still synthetic-only; `inference/train_model.py`'s `train_flow_autoencoder()` lets a client retrain it on their own baseline traffic — the same 'Learning Mode' idea enterprise tools like Darktrace use — before trusting it against novel behavior on someone else's network."
