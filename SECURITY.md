...
**Current measured result (hybrid model, see below): 85.2% precision,
69.8% recall, 12.0% false-positive rate, 78.9% overall accuracy.**
Per-family recall still varies (100% on `corebot`/`symmi`, 13-27% on
`vawtrak`/`conficker`/`pushdo`) — the script prints the full breakdown.
...
`hiyter.com`, `communicationfirewall-security.cc`). The model now
catches 5 of 7 (was 1 of 7 before this retrain), and no longer
false-positives on the real Microsoft telemetry/service-discovery
domains in that same capture that it previously flagged.

**How it got here — three real, sequentially-discovered bugs, not one clean retrain:**
...
2. Rebuilt the model as a hybrid (see architecture below) and retrained
   ...
   **93%** — nearly every real benign domain got flagged. Root cause,
   ...
   `inference/train_model.py`). Result: the numbers at the top of this
   section.
...
**Honest trade-off:** recall nearly doubled and precision held roughly
steady, but false-positive rate did rise (5.9% → 12.0%), and a few
character-random families that scored well before (`vawtrak`,
`conficker`, `pushdo`) now score worse — the training mix shifted
toward dictionary/hyphenated patterns to close that specific gap, and
some of that came at those families' expense. Net accuracy is still up
12.7 points (66.2% → 78.9%), but this is a real trade-off, not a
strict improvement on every axis.
