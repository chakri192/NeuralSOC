...
**Current measured result:** 86.5% precision, 38.1% recall, 5.9%
false-positive rate, 66.2% overall accuracy. Per-family recall varies
widely (98%+ on `corebot`/`rovnix`, near 0% on `suppobox`/`gozi`) — the
script prints the full breakdown. The two families it misses almost
entirely are dictionary-based DGAs, deliberately designed by their
authors to generate real-word-like domains specifically to evade
character-pattern detection like this; that the model struggles there
specifically, rather than uniformly, is itself a sign the number is
measuring something real rather than an artifact of the test.

This replaced a materially worse, previously undiscovered bug: the
model's benign training data was exactly 10 hardcoded famous domains
(`google.com`, `apple.com`, ...), which produced a classifier that was
99.9% recall but a **98% false-positive rate** the moment it was run
against real, diverse domains — it had learned "matches one of these 10
exact strings," not anything resembling what a legitimate domain looks
like. Retraining against 40,000 real domains sampled across Tranco's
full popularity range (not just the top few hundred) fixed the training
data, but the *first* retrain collapsed to 0% recall and 0% false
positives — it learned to predict "not DGA" for everything. Root cause:
`train_to_max()`'s training loop did one full-batch gradient step per
"epoch," capped at 50 total optimizer steps for the whole run — enough
signal for the old, trivially-separable 10-domain task, nowhere near
enough for a genuinely harder, realistic one. Switched to real
mini-batch training (`inference/train_model.py`, matching the pattern
`scripts/train_dl_models.py` already used correctly) before it produced
the numbers above.

The flow autoencoder has no equivalent real-world benchmark yet — no
public, labeled real-flow anomaly dataset was readily available in the
time this validation pass had; it remains trained and tested only
against the synthetic generator described in
[inference/train_model.py](inference/train_model.py)'s
`generate_benign_flow_dataset()`. Extending this same real-data
validation discipline to it is the natural next step.
